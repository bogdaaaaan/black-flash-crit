...
namespace BlackFlashCrit {
	...
	internal static class CritSettings {
		...
		internal static void Init (ConfigFile config) {
			...
			// Silk on crit config
			SilkOnCritEnabled = config.Bind("Crit", "Enable Silk On Crit", false, "If true, regain silk on critical hits (non-player victims).");
	...
	internal static class AudioSetting {
		...
		internal static void Init (ConfigFile config) {
			...
			MuteDefaultCritSfx = config.Bind("Audio", "Mute Default Critical Sound", true, "Mutes the game's default critical hit sound.");
			...
			MaxVoices = config.Bind("Hidden", "Max Simultaneous Voices (ADVANCED, hidden from UI).", 4, new ConfigDescription("Number of overlapping crit sounds allowed.", new AcceptableValueRange<int>(1, 16), new BrowsableAttribute(false)));
		}
		internal static void Update () {
